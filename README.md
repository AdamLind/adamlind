# Hi! I'm Adam Lind 👋

## 👨‍💻 About Me
* Software Development student at **BYU-Idaho**.
* Full-stack developer specializing in the **JavaScript/TypeScript ecosystem** and **.NET/C#**.
* Focused on building scalable **Next.js** and **React Native** applications.
* Passionate about creating **efficient, performance-driven solutions**.
* 🔗 [LinkedIn](https://www.linkedin.com/in/adam-lind-489474236/)
 
## 🛠️ Core Tech Stack

### 🌐 Languages
[![Languages](https://skillicons.dev/icons?i=js,ts,python,cs,html,css)](https://skillicons.dev)

### 🖥️ Frontend & UI
[![Frontend](https://skillicons.dev/icons?i=react,nextjs,tailwindcss,rn)](https://skillicons.dev)

### ⚙️ Backend & Runtimes
[![Backend & Runtimes](https://skillicons.dev/icons?i=nodejs,dotnet,graphql,apollo)](https://skillicons.dev)

### 💾 Databases & ORMs
[![Databases & ORMs](https://skillicons.dev/icons?i=mongodb,postgresql,mysql,prisma)](https://skillicons.dev)

### ☁️ Tools & DevOps
[![Tools & DevOps](https://skillicons.dev/icons?i=git,vscode,npm,docker,vercel)](https://skillicons.dev)

 
## 📈 GitHub Stats

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=AdamLind&show_icons=true&theme=radical)](https://github.com/anuraghazra/github-readme-stats)
 
## 🔥 Current Projects & Focus
* **Artisan Marketplace**
    * Full-stack e-commerce platform for handmade goods.
    * **Tech Stack:** Next.js, TypeScript, Tailwind CSS, PostgreSQL, Prisma.
    * **Features:** Product listings, secure user authentication, seller dashboards.
* **React Native/Python Automation**
    * Developing mobile applications and utility scripts for automation and data processing.
* **C#/.NET Integration**
    * Exploring API design using C# and the .NET framework for high-performance microservices.
 
## 💡 Learning & Goals
* **Advanced Next.js:** Mastering React Server Components and data fetching patterns.
* **Data Optimization:** Deep dive into PostgreSQL indexing and MongoDB aggregation pipelines.
* **Software Design:** Focusing on architecture, design patterns, and test-driven development (TDD).
* **2025 Goal:** Build and launch 3 production-ready, full-stack applications.
 
## 🕒 Time Spent This Week
* ████████████░░░░   65% React Native
* ███░░░░░░░░░░░░░   15% PostgreSQL
* ██░░░░░░░░░░░░░░   12% Tailwind CSS
* █░░░░░░░░░░░░░░░    8% Other Studies
